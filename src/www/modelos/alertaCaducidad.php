...
class AlertaCaducidad {
    ...
    private function calcularDiasRestantes() {
        ...
        $this->dias_restantes = $diferencia->invert ? -$diferencia->days : $diferencia->days;
    }
    ...
    public function guardar() {
        try {
            ...
            $sql = "INSERT INTO alertas_caducidad (nombre_producto, punto_distribucion_id, cantidad, fecha_caducidad, ubicacion, estado)
                    VALUES (:nombre_producto, :punto_distribucion_id, :cantidad, :fecha_caducidad, :ubicacion, :estado)";
            $params = [
    ...
    public static function listar() {
        try {
            $bd = new BD();
            self::asegurarTabla($bd);
            $filas = $bd->seleccionar("SELECT * FROM alertas_caducidad ORDER BY fecha_caducidad ASC, id DESC");
            $alertas = [];
    ...
    public static function obtenerPorId($id) {
        try {
            $bd = new BD();
            self::asegurarTabla($bd);
            $filas = $bd->seleccionar("SELECT * FROM alertas_caducidad WHERE id = :id", [':id' => $id]);
    ...
    public static function eliminar($id) {
        try {
            $bd = new BD();
            self::asegurarTabla($bd);
            $bd->ejecutar("DELETE FROM alertas_caducidad WHERE id = :id", [':id' => $id]);
            return true;
        } catch (Throwable $exception) {
    ...
    public static function obtenerAlertas($filtro = 'todas') {
        try {
            ...
            throw new Exception("Error al filtrar alertas: " . $exception->getMessage());
        }
    }
}
?>
